<x-app-layout>
    ...
    <div class="py-12">
        ...
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <form action="{{ route('persona.store') }}" method="POST" enctype="multipart/form-data" data-parsley-validate>
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                    <div>
                        ...
                        <input id="nombre" name="nombre" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text">
                       
                    </div>
                    <div>
                        <label class="text-gray-700" for="edad">Edad de la persona</label>
                        <input id="edad" name="edad" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="number">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-6 mt-4">
                    ...
                    <div>
                        ...
                        <select id="id_gatito" name="id_gatito" class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-600 block w-full p-2.5">
                            ...
                            <option value="{{ $gato->id }}" {{ old('id')==$gato->id?'selected':'' }}>
                                {{ $gato->nombre }}
                            </option>
                            @endforeach
                        </select>
